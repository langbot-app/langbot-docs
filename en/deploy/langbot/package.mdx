...
To try Beta 3 instead, use the command below. `--prerelease=allow` also permits prereleases of dependencies; `--refresh` refreshes cached package data.

```bash
uvx --prerelease=allow --refresh langbot==4.11.0b3
```
