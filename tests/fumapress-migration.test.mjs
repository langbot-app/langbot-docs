...
test("canonical MDX and nested OpenAPI sources are discovered", async () => {
  ...
  assert.equal(pages.length, 317);
  assert.deepEqual(Object.fromEntries([...pages.reduce((acc, page) => acc.set(page.split("/", 1)[0], (acc.get(page.split("/", 1)[0]) ?? 0) + 1), new Map()).entries()]), { en: 101, zh: 115, ja: 101 });
...
test("prebuild is deterministic and preserves local assets and SEO files", async () => {
  ...
  try {
    ...
    assert.equal(first.documents, 317);
